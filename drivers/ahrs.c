...
float volatile roll = 0.0f, pitch = 0.0f, yaw = 0.0f;
...
void AHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float dt) {
	...
	if((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f)) {
		AHRSupdateIMU(gx, gy, gz, ax, ay, az, dt);
		return;
	}
...
{ 
  ...
  roll = atan(gy / sqrt(gx*gx + gz*gz))  * 180/M_PI - roll_angle_off_set;
  pitch = -atan(gx / sqrt(gy*gy + gz*gz))  * 180/M_PI - pitch_angle_off_set;
